		...
		chatCompletion: {
			POST: async ({ request }) => {
				...
				try {
					...
					const assistantPrompt = prompts.assistant(
						documentContent || TEMP_CONTENT,
						name || TEMP_USER
					);
					...
					messages.slice(-4).forEach((msg) => {
						if (msg.role === 'assistant')
							return recentChatHistory.push(new AIMessage(String(msg.content)));
						if (msg.role === 'user')
							return recentChatHistory.push(new HumanMessage(String(msg.content)));
